use sqlx::Executor;
use uuid::Uuid;
use zero2prod::configuration::{DBPool, DatabaseSettings, Settings};
...
impl TestApp {
    ...
    async fn spawn_app() -> Self {
        ...
        let db_executor = configure_database(&configuration.database).await;

        let server =
            zero2prod::startup::run(listener, db_executor.clone()).expect("Failed to run server.");
