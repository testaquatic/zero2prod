...
impl SubscriberName {
    ...
    pub fn parse(s: String) -> Result<Self, String> {
        ...
        let is_empty_wor_whitespace = s.trim().is_empty();
        ...
        let forbidden_charactors = ['/', '\\', '(', ')', '"', '<', '>', '{', '}', '[', ']'];
        let contains_forbidden_charactors = s.contains(forbidden_charactors);

        // 어느 한 조건이라도 위반하면 `false`를 반환한다.
        if is_empty_wor_whitespace || is_too_long || contains_forbidden_charactors {
